...
$ ./resnet50
```
とすると画像認識デモが開始し、様々な画像が次々表示されるとともに、標準出力に各画像の認識結果と実行時の性能が表示されます。手元の環境では平均200 GOPS程度の演算性能が確認できました。
...
```
とすることで、セグメンテーションのデモアプリケーションが実行されます。動画内の車両や交通標識が再生中に塗り分けられます。手元でFPS測定のコードを追加し検証したところ、26.1 FPS程度の値が得られました。
